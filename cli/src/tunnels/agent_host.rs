...
impl AgentHostManager {
	...
	pub async fn get_latest_release(&self) -> Result<Release, CodeError> {
		...
		if let Some((_, release)) = &*latest { return Ok(release.clone()); }
		let quality = VSCODE_CLI_QUALITY
			.ok_or(CodeError::UpdatesNotConfigured("no configured quality"))
			.and_then(|q| Quality::try_from(q).map_err(|_| CodeError::UpdatesNotConfigured("unknown quality")))?;
		let commit: String = option_env!("VSCODE_CLI_COMMIT").unwrap_or("6928394f91b684055b873eecb8bc281365131f1c").to_string();
		let release = Release { name: String::new(), commit, platform: self.platform, target: TargetKind::Server, quality };
		let now = Instant::now();
		info!(self.log, "Server pinned: {}", release);
		*latest = Some((now, release.clone()));
		Ok(release)
	}

	pub async fn run_update_loop(self: Arc<Self>) {
		info!(self.log, "Update loop disabled; server pinned.");
	}
	...
	async fn handle_management_request(
		...
		req: Request<Incoming>,
			...
			// Upgrade route disabled in no-update build
